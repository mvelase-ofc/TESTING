...
const REPO_NAME = 'MVELASE-MD-BOT';
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        let pluginList = "📂 *MVELASE BOT Plugins:*\n\n";
...
async (conn, mek, m, { from, reply, args, senderNumber }) => {
    try {
        ...
        if (m.quoted && m.quoted.key.fromMe) {
            ...
            if (/📂 \*MVELASE BOT Plugins:\*/i.test(quotedText)) {
        ...
        const statusMessage = {
            image: { url: `https://files.catbox.moe/8ub6ps.png` }, // Replace with your image URL
